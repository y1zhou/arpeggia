...
def test_import():
    ...
    assert hasattr(arpeggia, "sequences")
...
def test_contacts(test_pdb_file):
    ...
    assert len(df) > 0, "Contacts DataFrame should not be empty"
    ...
    for col in expected_columns:
        assert col in df.columns, f"Column '{col}' should be present in contacts DataFrame"
...
def test_sasa(test_pdb_file):
    ...
    # Check DataFrame is not empty
    assert len(df) > 0, "SASA DataFrame should not be empty"
    
    # Check expected columns exist
    expected_columns = ["atomi", "sasa", "chain", "resn", "resi", "insertion", "altloc", "atomn"]
...
def test_sequences(test_pdb_file):
    """Test the sequences function returns expected structure."""
    import arpeggia
    
    seqs = arpeggia.sequences(test_pdb_file)
    ...
    for chain_id, seq in seqs.items():
        ...
        assert seq.startswith(expected_start), f"Sequence should start with {expected_start}, got {seq[:10]}"
...
def test_error_handling():
    ...
    with pytest.raises(Exception):
        arpeggia.contacts("nonexistent_file.pdb")
    
    with pytest.raises(Exception):
        arpeggia.sasa("nonexistent_file.pdb")
    
    with pytest.raises(Exception):
        arpeggia.sequences("nonexistent_file.pdb")
