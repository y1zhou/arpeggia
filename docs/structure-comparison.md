...
traditional fit-and-score-the-same-atoms calculation. An empty `rmsd_residues`
does **not** inherit `superpose_residues`. `model_num=0` (CLI `--model 0`) selects
the first model independently in each structure; other values select model serials.
...
Arbitrary polymers, ligands, and unrecognized modified residues are not retained
even in `all` mode. Sequence-aware RMSD excludes caps without correspondence.
...
Ranges are inclusive. Repeat the chain in each clause (`A:1,A:3`, not `A:1,3`).
`A:10` includes all insertion variants, while `A:10A` selects only insertion A.
Blank chain IDs cannot be addressed explicitly by this grammar. With
`rmsd --align-seqs`, both selectors use reference author numbering; otherwise
they apply to both structures.
