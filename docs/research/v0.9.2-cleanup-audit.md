...
Chain Groups Specification is under Usage; links beside Features point to the
[conventions and contact decision path](../scientific-conventions.md) and the
[structure RMSD and clustering guide](../benchmarks/structure-clustering.md).
The table distinguishes scalar/sequence results from DataFrames.
...
The second pass starts from `4481556` and follows the agreed review decisions.
...
`/usr/bin/time`; it includes setup and is not an allocation-only measurement.
...
The local source snapshot, isolated Rust harness, saved original ring method,
`buffered`/`streamed` binaries, output comparisons, and raw measurements remain
under `/tmp/arpeggia-ring-benchmark/`. `prepare.py` copies production source into
the snapshot and adds benchmark-only entry points; production APIs are unchanged.
The saved `old-ring-method.rs` supplies the buffered variant. Measurements rerun
with:

```bash
python3 /tmp/arpeggia-ring-benchmark/run.py
python3 /tmp/arpeggia-ring-benchmark/run.py --6bft
python3 /tmp/arpeggia-ring-benchmark/check_outputs.py
```
