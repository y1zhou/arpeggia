...
labels and operation styles. Display rulers now use numeric source positions
instead of allocating a string for every numbered residue; one reference-derived
CDR band map serves the whole block. Summary wrapping counts visible text before
applying styles; tied gene/allele names are grouped without discarding source
records. Reference position conversion is shared by
imputation and rendering; comparison coverage bounds are computed once per row.
